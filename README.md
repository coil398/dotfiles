...
cursor / opencode / shared-drift / codex-private の各契約テストをまとめて実行する集約ランナー:

```sh
bash etc/test-all-contracts.sh
```

`test-cursor-contracts.sh`（`sync-cursor --check` を含む）、`test-opencode-contracts.sh`（`sync-opencode --check`・冪等性・agent 変換契約・孤児削除を含む）、`check-shared-drift.sh`、`test-codex-private-contract.sh`（専用 profile / launcher / runtime link / `$HOME/bin` の非破壊展開）を実行し、どれが PASS/FAIL したかを集計表示する。どれかが失敗しても残りは実行され（fail-fast しない）、1 本でも FAIL なら終了コード 1 を返す。
