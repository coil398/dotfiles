...
from jev_hooks import jev_client, policy, redact  # noqa: E402
from jev_hooks.jev_client import ChoiceAnswer  # noqa: E402
from jev_hooks.transcript import ToolRecord, TurnContext, UserMessage  # noqa: E402
...
class CombineTests(unittest.TestCase):
    ...
    def test_missing_answer_is_a_remaining_deliverable(self) -> None:
        d = policy.combine(answers(rk="explain_or_plan_only"), self.T)
        self.assertEqual((d.verdict, d.reason_code), (policy.CONTINUE_WORK, "OK"))

    def test_answer_delivered_and_genuine_question_allow_stop(self) -> None:
        self.assertEqual(policy.combine(answers(rk="explain_or_plan_only", rw="complete"), self.T).verdict, policy.ALLOW_STOP)
        self.assertEqual(policy.combine(answers(rk="explain_or_plan_only", bl="needs_user"), self.T).verdict, policy.NEEDS_USER)
