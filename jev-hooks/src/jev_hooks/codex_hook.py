...
from . import VERSION, jev_client, logbook, policy, service
...
def _base_record(payload: Dict[str, Any], cfg: Config, started: float) -> Dict[str, Any]:
    return {
        ...
        "_started": started,
        "_cfg": cfg,
        "runtime": payload.get("_runtime", "codex"),
    }


def _finish(record: Dict[str, Any], output: Dict[str, Any]) -> Outcome:
    cfg = record.pop("_cfg", None)
    evaluated = record.pop("_api_evaluated", False)
    if cfg is not None and cfg.mode != "off" and not evaluated:
        service.record_skip("completion", record.get("reason_code", "SKIPPED"), cfg=cfg,
                            runtime=record.get("runtime", "codex"), event="Stop",
                            session_id=record.get("session_id") or "", turn_id=record.get("turn_id") or "")
...
def evaluate(
    ...
    *,
    ...
    api_key, source = resolve_api_key(environ)
    record["api_key_source"] = source
    if not api_key:
...
def _evaluate_locked(
    ...
    state_obj["execution_mode"] = payload.get("permission_mode", "default")
    state_obj["scope_note"] = "In plan/ask mode the deliverable is an answer or plan, never implementation. Do not request changes outside the authorized mode."
    ...
    evaluation = service.evaluate(
        "completion", state_obj, policy.QUESTIONS, cfg=cfg, environ=environ,
        runtime=record.get("runtime", "codex"), event="Stop",
        session_id=payload["session_id"], turn_id=turn_id,
        ask_fn=ask_fn, timeout_s=api_timeout, cwd=payload.get("cwd"),
    )
    record["_api_evaluated"] = True
    if evaluation.status != "ok" or evaluation.result is None:
        persist(policy.SKIPPED)
        return _skip(record, evaluation.reason)
...
def main(argv: Optional[list] = None) -> int:
    ...
    if args and args[0] == "--install-codex-hook":
        ...
        changed=install_codex_hook(home,Path(__file__).resolve().parents[3]/".codex/config.toml")
    ...
    if args and args[0] == "--trust-codex":
        ...
        from .trust import apply_hooks_trust
        ...
        try:
            changed, digest = apply_hooks_trust(target, key_source=key_source)
    ...
    if args and args[0] in ("-h", "--help"):
        sys.stdout.write(
            "usage: codex-hook.py [--install-codex-hook [CODEX_HOME] | --doctor | --explain | --trust-codex [config.toml]]\n"
            "  (no args)     read Codex Stop payload on stdin, write hook output JSON\n"
            "  --install-codex-hook [home] install the Stop hook for a custom CODEX_HOME\n"
