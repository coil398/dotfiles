...
- `etc/link.sh` の `codex-private` launcher 展開は `$HOME/bin` を置換せず、`$HOME/bin/codex-private` だけを管理 symlink にする。同名の非 symlink target は削除・上書きせず fail closed する
...
- **private Unity 専用入口** — `$HOME/bin/codex-private` は `private-private` root からだけ起動でき、両リポジトリの `AGENTS.md` と private 側 `scripts/private.sh` を検証したうえで `codex -p private -C <private-root> --add-dir <private_app>` を実行する。`private` profile は opt-in の `danger-full-access` + `approval_policy = "never"` で、通常の `codex` の安全設定は変更しない。専用セッションでも Unity 操作は `private.sh` 経由に限定する
- **private profile の runtime link** — `.codex/private.config.toml` は hand-written native profile とし、`etc/link-codex-runtime.sh` が `~/.codex/private.config.toml` へ管理 symlink を作る。生成済み `.codex/config.toml` や global default に混ぜない
