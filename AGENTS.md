...
- `.codex/AGENTS.md` / `.codex/config.toml` / `~/.config/opencode/**` の生成物だけが変わっている場合は、手書き編集や再生成漏れを疑う
...
- Shared instructions for Codex/OpenCode adapters: `AGENTS.md`
- Shared skill core: `.agents/skills/*/SKILL.md`
- MCP servers: `mcp-servers.json`
- Tool-specific native overlays are allowed and expected. Do not force exact behavioral parity when Claude Code, Codex, and OpenCode benefit from different mechanics
