...
#
# NOTE: Servers not present in mcp-servers.json, or marked with a foreign
# *Only flag (openCodeOnly / codexOnly / cursorOnly / devinOnly), are
# automatically removed from Claude Code user scope on each sync. Any
...
# Remove orphan / foreign *Only servers from Claude Code user scope.
...
# - Servers marked *Only:true for another runtime → remove from Claude Code
ssot_names=$(jq -r '.mcpServers | to_entries | map(select([.value.openCodeOnly, .value.codexOnly, .value.cursorOnly, .value.devinOnly] | any | not)) | .[].key' "$CONFIG_FILE" | sort -u)
if [ -f "${HOME}/.claude.json" ]; then
  ...
  for n in $current_names; do
    if ! echo "$ssot_names" | grep -qx "$n"; then
      echo "[sync-mcp] removing orphan/foreign-only server from Claude Code user scope: $n"
      claude mcp remove "$n" -s user >/dev/null 2>&1 || echo "[sync-mcp] warn: failed to remove $n"
    fi
  done
fi

names=$(jq -r '.mcpServers | to_entries | map(select([.value.openCodeOnly, .value.codexOnly, .value.cursorOnly, .value.devinOnly] | any | not)) | .[].key' "$CONFIG_FILE")
