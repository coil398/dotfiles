...
from jev_stop_guard import codex_hook, policy  # noqa: E402
from jev_stop_guard.config import Config  # noqa: E402
from jev_stop_guard.jev_client import ChoiceAnswer, JevResult  # noqa: E402
from jev_stop_guard.state import StateStore  # noqa: E402
...
class LimitTests(unittest.TestCase):
    ...
    def test_state_write_failure_fails_open(self) -> None:
        ...
        def broken_save(state):
            from jev_stop_guard.state import StateError
...
class SkipGateTests(unittest.TestCase):
    ...
    def test_plan_mode_allows_without_api(self) -> None:
        h = self.h
        p = h.payload("s", "t1", "a", active=False)
        p["permission_mode"] = "plan"
        o = h.run(p)
        self.assertEqual((o.record["verdict"], o.record["reason_code"]), (policy.ALLOW_STOP, "PLAN_MODE"))
        self.assertEqual(h.jev.calls, 0)
    ...
    def test_api_key_from_zsh_secret(self) -> None:
        h = self.h
        h.transcript("s", turn_lines("t1"))
        ...
        o = h.run(h.payload("s", "t1", "a", active=False))
        self.assertTrue(o.blocks)
        self.assertEqual(o.record["api_key_source"], "secret_file")
