#!/usr/bin/env bash
# 契約テスト集約ランナー。cursor / opencode / shared-drift / codex-private の契約をまとめて実行する。
#
...
if [ "$cursor_status" = "PASS" ] && [ "$opencode_status" = "PASS" ] && [ "$drift_status" = "PASS" ] && [ "$private_status" = "PASS" ] && [ "$antigravity_status" = "PASS" ]; then
  echo "ALL PASS"
  exit 0
else
  echo "SOME FAILED"
