...
- **Task `model`**: omit or `inherit` only (follow parent Auto). Do not pass vendor slugs (`opus` / `sonnet` / `claude-*` / `cursor-grok-*`, etc.) unless the user explicitly names a model.
