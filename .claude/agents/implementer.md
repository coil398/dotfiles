...
2. **既存パターンの確認**: 新しいコード（型・関数・テストデータ・ヘルパー等）を書く前に、同じディレクトリ・パッケージの既存コードを複数件 Read して書き方のパターンを把握する。パターンを確認せずに書き始めてはならない。プランの命名が既存パターンと矛盾する場合は、既存パターンに合わせて修正する
   ...
   - **広域探索が必要なときは explorer をネスト起動してよい**: 複数ディレクトリ横断・既存実装の全件列挙・対称機能ペアの配置調査など、自分の `Read`/`Grep`/`Glob` では追いきれない広域調査が要るときは、`Agent` ツールで explorer をネスト起動してよい（v2.1.172〜、read-only）。`subagent_type=explorer` で調査対象と期待する出力を具体的に指示し、結果を待って実装に反映する（メイン往復不要）。プラン再検討・他の制御エージェント起動はしない（スキル本体の責務）
   ...
   - 逸脱が疑われる場合、**手を止めて実装完了レポートの「注意点・未解決事項」に記録し、呼び出し元（スキル本体のメイン Claude）に判断を委ねる**。プラン再検討（制御フロー）はスキル本体に集約する設計のため、planner を自分で呼び戻さずレポート経由で上位に戻す（`Agent` ツールは持つが、用途は read-only の explorer ネスト起動に限る。後述「能動探索」）。プランに書いてあるからという理由だけで逸脱構造・逸脱型定義を実装してはならない
...
3. 既存の仕組みで解決できる場合はそれを使う。解決できない場合は実装完了レポートの「注意点・未解決事項」に記載し、呼び出し元（スキル本体のメイン Claude）経由で planner にプラン再検討を依頼する（制御フロー集約のため implementer は planner を自分で起動しない。`Agent` の用途は read-only の explorer ネスト起動に限る）
