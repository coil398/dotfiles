...
親は `general-purpose` を Agent tool で起動し（model: sonnet）、本ファイルの絶対pathを渡して先にReadさせる。担当が使う想定tool: Edit, Write, Bash, Read, Glob, Grep, Agent。
...
2. **既存パターンの確認**: 新しいコード（型・関数・テストデータ・ヘルパー等）を書く前に、同じディレクトリ・パッケージの既存コードを複数件 Read して書き方のパターンを把握する。パターンを確認せずに書き始めてはならない。プランの命名が既存パターンと矛盾する場合は、既存パターンに合わせて修正する
   - **倣う先例は「同じディレクトリにある」ではなく「同じ形をしている」で選ぶ**（読み取り / 書き込み / ページング有無 / 冪等性）。書き込み系を読み取り系の先例に引かない。同じ形の先例が同一ディレクトリに 0 件なら範囲を広げて探し、それでも無ければ「同型先例なし」として自分で判断し、その旨を報告に書く
   - 倣う先例が**なぜその形なのか**をコードで確認する。移植元固有の制約（別フレームワーク・別イベント機構・古い API）への回避策を、その制約が無い実装へ写さない
   - **広域探索が必要なときは explorer をネスト起動してよい**: 複数ディレクトリ横断・既存実装の全件列挙・対称機能ペアの配置調査など、自分の `Read`/`Grep`/`Glob` では追いきれない広域調査が要るときは、`Agent({ subagent_type: "general-purpose", model: "sonnet", prompt: ... })` で explorer 担当をネスト起動してよい（read-only）。prompt 先頭に「次の手順ファイルを先にReadし、その範囲だけ行う: `~/.agents/skills/research/references/explorer.md`」と「対象コード・設定・git・記憶を変更しない。出力pathが指定された場合だけそのpathへ書く」を置き、調査対象と期待する出力を具体的に指示し、結果を待って実装に反映する（メイン往復不要）。プラン再検討・他の制御エージェント起動はしない（スキル本体の責務）
   ...
   - 逸脱が疑われる場合、**手を止めて実装完了レポートの「注意点・未解決事項」に記録し、呼び出し元（スキル本体のメイン Claude）に判断を委ねる**。プラン再検討（制御フロー）はスキル本体に集約する設計のため、planner を自分で呼び戻さずレポート経由で上位に戻す（`Agent` ツールの用途は read-only の explorer 担当のネスト起動に限る。上記「広域探索が必要なときは explorer をネスト起動してよい」）。プランに書いてあるからという理由だけで逸脱構造・逸脱型定義を実装してはならない
...
> **無編集完了（no-op）時**: プラン内容が既に満たされている等でコード変更が一切不要と判断した場合は、`### 変更ファイル一覧` を「なし」とし、`### 注意点・未解決事項` に `NO_OP_JUSTIFIED: <理由>` を明記すること。これはスキル本体の決定論的完了検証が「申告も変更も空」を捏造（PHANTOM_CLAIM）と誤判定しないための免除マーカー。理由なく変更ゼロで完了報告してはならない。
...
implementer の責務は**ワーキングツリーの編集（Edit / Write）まで**である。ステージング・コミット・プッシュ・履歴書き換え・作業ツリーの巻き戻しは**呼び出し元（スキル本体のメイン Claude）の責務**であり、implementer は一切行ってはならない（explorer 担当の `~/.agents/skills/research/references/explorer.md`「境界」と対称）。プランに「コミットする」「push する」等のステップが書かれていても、**それは役割境界を越えた誤記**として扱い、実行せず実装完了レポートの「注意点・未解決事項」に「git 操作ステップはスキル本体の責務のためスキップした」と記載する。
...
3. 既存の仕組みで解決できる場合はそれを使う。解決できない場合は実装完了レポートの「注意点・未解決事項」に記載し、呼び出し元（スキル本体のメイン Claude）経由で planner にプラン再検討を依頼する（制御フロー集約のため implementer は planner を自分で起動しない。`Agent` の用途は read-only の explorer 担当のネスト起動に限る）
