...
3. planner が出した各 topic ごとに explorer を起動する（topic が独立なら並列起動可、上限は CLAUDE.md の「コードベース探索の委譲」に従う）:
...
2. **フラグ未指定時のデフォルト**: 全 5 観点 `[correctness, consistency, quality, security, architecture]`（planner が動くタスクは設計判断を含むため）。詳細プロトコル: `~/.claude/skills/pir2/references/reviewer-set-algorithm.md` を参照
...
観点マッピング（REVIEWER_ROLE ごとの担当分野）・違反パターンは `~/.claude/skills/pir2/references/fan-out-gate.md` を参照。
...
## ステップ 5.4: リファクタ提案（refactor-advisor 起動 → ゲート → 任意適用）

全体 VERDICT が PASS の場合のみ実行。LOOP_COUNT 上限到達で FAIL 確定の場合はスキップしてステップ 5.5 へ。

詳細プロトコル: `~/.claude/skills/pir2/references/refactor-advisor-gate.md` を参照。

debug での適用ポイント:
- refactor-advisor 1 体起動、提案存在確認、ユーザーゲート提示、ユーザー選択処理は通常 pir2 と同じ
- リファクタ適用時の implementer 再起動後は **ステップ 4-2A（Fan-Out Gate 宣言）→ 4-2B（並列発火）** で reviewer を同じ REVIEWER_SET で再起動（退行検知）
- 再 reviewer FAIL なら ステップ 5 の差し戻しループに合流（`LOOP_COUNT` 継続）
- refactor-advisor は同一 run 内 **1 回のみ**起動
