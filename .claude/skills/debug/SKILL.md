---
name: debug
description: エラーや不具合を、ログ・再現・コードの実測から診断して修正する。症状、stack trace、error logがある調査で使う。`--deepplan`は診断計画を深める。
...
```bash
PROJECT_ROOT="$(pwd)"
# sanitized-cwd 計算（PROJECT_MEMORY_DIR 用）は ~/.claude/skills/pir2/references/sanitized-cwd.md を SSOT とする
...
# handoff.md はプロジェクト単位で1ファイル（per-run ではない。詳細: ~/.claude/pir-handoff.md）
...
Skill `deepplan` を同一 `RUN_DIR` で起動。プロンプトに「デバッグタスクであること」「plan.md 冒頭に診断セクション（file:line 証拠必須）」を追加で渡す。
...
- [ ] ステップ 5.9: 診断レポートの docs/debug/ への保存
- [ ] ステップ 6: 最終サマリーの提示
...
`{PROJECT_ROOT}/docs/debug/` ディレクトリがなければ作成してください。

`{RUN_DIR}/plan.md`（診断・修正プラン）、最新の `{RUN_DIR}/implementation-*.md`（実装内容）、`{RUN_DIR}/review-*-*.md`（レビュー結果）を Read し、以下の形式で `{PROJECT_ROOT}/docs/debug/${run_ts}-${run_feature}.md` に Write してください:
...
[plan.md の「## 診断: [根本原因]」セクションをそのまま転記。file:line と該当コード引用を含む]

## 修正内容

[implementation-*.md から変更ファイル・変更内容を転記]

## レビュー結果

- 最終 VERDICT: [PASS/FAIL]
- REVIEWER_SET: [起動した観点のカンマ区切り]
- ループ回数: [LOOP_COUNT]
...
### 診断レポート
{PROJECT_ROOT}/docs/debug/${run_ts}-${run_feature}.md
