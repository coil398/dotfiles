...
PIR² 系スキル共通の tester 起動仕様。スキル本体（メイン Claude）は tester を起動する **前に** TEST_SCOPE を構築してプロンプトに明示渡しする。
...
3. **TEST_SCOPE を最終決定（マトリクス）**
   ...
   - **フルスイートのコマンド出典**: プロジェクト CLAUDE.md の Testing セクションで定義されているもの（例: motitan-api なら `go test ./... -shuffle=on -timeout=20m`）を使う。SKILL.md / tester-prompt.md に直書きしない
