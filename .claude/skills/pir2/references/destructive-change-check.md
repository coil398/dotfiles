...
PIR² スキル（/pir2）のステップ 5.7。implementer 起動前に、メイン Claude（スキル本体）が plan.md と explorer レポートを Read して **2 軸の機械チェック** を行う。
...
変更の波及と動作影響に応じて reviewer / tester の範囲を選び、必要な確認の省略と過剰なフルスイート実行を防ぐ。
