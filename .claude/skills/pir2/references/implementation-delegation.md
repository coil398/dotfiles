...
- 「ステージング・コミット・プッシュ・履歴/作業ツリー変更（`git add -A`/`git add .`/個別 `git add`/`git commit`/`git push`/`git reset`/`git restore`/`git stash` 等）は禁止。git はワーキングツリーの Edit/Write までで止める。ステージング/コミット/push は呼び出し元（スキル本体）の責務。読み取り系（`git status`/`git diff`/`git log`/`git show` 等）は自己検証に使ってよい（`implementer.md`「## 禁止: コミット・プッシュ・`git add -A`・履歴/作業ツリー変更」参照）」
