...
- Eligible decisions: 2（実 run 2 件。いずれも一方向の密結合で shard 不可・直列 unit 化が適合したケース。固有名を含む詳細は `~/.claude/memory/experimental_observations.md`）
- Sequential executions: 2（fresh implementer で context-rot 回避、後続 unit が先行 unit を git diff で読んで命名整合を維持）
- Unit-boundary regressions: 0（1 件目: 軽微な前方参照 friction のみ＝共有純ヘルパーの定義層のズレで解消可能。2 件目: 統合不整合ゼロで review-01 が全 6 観点初回 PASS。いずれも命名ドリフト起因の統合不整合ではない）
- Recommendation changes: 0（2 executions。採用条件「3 回以上」未達のため Continue observing 継続）
...
- Initial implementation-point executions: 8（4 は epic-A の4サブ pir2 初回実装点、2 は epic-B の T1+T2 サブタスク初回実装点＝いずれもネスト general-purpose ランナー経由、2 は **非 epic standalone /pir2 run（L0=メイン Claude 直接）の初回実装点**＝ネストを介さない直接 SKILL.md 経路〔別々の 2 プロジェクトの standalone run 各 1 件〕。固有名を含む詳細は `~/.claude/memory/experimental_observations.md`）
...
- 6-3 execution audit misses (verify-*.list missing despite claimed completion): 4（すべて epic-A の**4 サブ run** に集中。audit hit は 4（epic-B T1+T2 サブタスク + 非 epic standalone run 2 件）。原因未確定: (i) epic-A サブ run が本チェック追加 [2026-07-04 開始] より前の SKILL.md 経路を辿った / (ii) ネスト general-purpose ランナーが 6-3 を省略、のいずれか。ネスト文脈で一律に 6-3 が省略されるわけではなく、epic/オーケストレーター実装の個体差（どちらの SKILL.md 経路・プロンプト構成を辿ったか）に依存する可能性が高い。加えて **非 epic standalone run（L0=メイン直接）で audit hit が 2 件連続**（別々の 2 プロジェクト）＝ネストを介さない直接 SKILL.md 経路では 6-3 が一貫して明確に発火し、miss は epic-nested runner 個体差に起因する仮説((ii))を 2 件目でさらに補強。採用条件 (b) の観察に計上）
- Recommendation changes: 0（Continue observing 継続。audit hit=4 / miss=4 で miss は epic-A のみに集中、standalone〔2/2〕・epic-B〔2/2〕では一貫して hit。次サイクル以降も「どの epic 実装がどちらの経路を辿るか」の切り分けを継続観察。epic-A 系で systematic な miss が続けば W2/W5 hook-floor の前倒し検討）
