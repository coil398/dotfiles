...
- Initial implementation-point executions: 9（4 は epic-A の4サブ pir2 初回実装点、2 は epic-B の T1+T2 サブタスク初回実装点＝いずれもネスト general-purpose ランナー経由、3 は **非 epic standalone /pir2 run（L0=メイン Claude 直接）の初回実装点**＝ネストを介さない直接 SKILL.md 経路〔3 件目を追加観測〕。固有名を含む詳細は `~/.claude/memory/experimental_observations.md`）
...
- 6-3 execution audit misses (verify-*.list missing despite claimed completion): 4（すべて epic-A の**4 サブ run** に集中。audit hit は 5（epic-B T1+T2 サブタスク + 非 epic standalone run 3 件）。原因未確定: (i) epic-A サブ run が本チェック追加 [2026-07-04 開始] より前の SKILL.md 経路を辿った / (ii) ネスト general-purpose ランナーが 6-3 を省略、のいずれか。ネスト文脈で一律に 6-3 が省略されるわけではなく、epic/オーケストレーター実装の個体差（どちらの SKILL.md 経路・プロンプト構成を辿ったか）に依存する可能性が高い。加えて **非 epic standalone run（L0=メイン直接）で audit hit が 3 件連続**＝ネストを介さない直接 SKILL.md 経路では 6-3 が一貫して明確に発火し、miss は epic-nested runner 個体差に起因する仮説((ii))を 3 件目でさらに補強。採用条件 (b) の観察に計上）
- Recommendation changes: 0（Continue observing 継続。audit hit=5 / miss=4 で miss は epic-A のみに集中、standalone〔3/3〕・epic-B〔2/2〕では一貫して hit。採用条件「10 回以上」まで残り 1 件。次サイクル以降も「どの epic 実装がどちらの経路を辿るか」の切り分けを継続観察。epic-A 系で systematic な miss が続けば W2/W5 hook-floor の前倒し検討）
