...
- Initial implementation-point executions: 14（4 は epic-A の4サブ pir2 初回実装点、2 は epic-B の T1+T2 サブタスク初回実装点＝いずれもネスト general-purpose ランナー経由、4 は **非 epic standalone /pir2 run（L0=メイン Claude 直接）の初回実装点**＝ネストを介さない直接 SKILL.md 経路、3 は **ネスト pir2 サブタスク T2・T3・T5〔別エピックのネスト T-chain・オーケストレーター=サブエージェント〕の初回実装点＝ネスト runner だが 6-3 が発火し audit HIT**〔T5 で delta=0 が3度目〕、1 は **同一 T-chain の後続サブタスク T-EXIT9＝ネスト runner だが 6-3 不発（audit MISS、14 件目）**。固有名を含む詳細は `~/.claude/memory/experimental_observations.md`）
