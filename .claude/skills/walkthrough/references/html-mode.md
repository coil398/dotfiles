...
`open` / `xdg-open` でブラウザを勝手に起動しない。ユーザーが「ブラウザで開いて」と言ったときだけ実行する。
...
| タイミング | 挙動 |
|---|---|
| ステップ6（冒頭ウォークスルー保存直後） | フル生成 → `SendUserFile` |
| ステップ7の詳細化ループ中 | **何もしない**。md にだけ追記する |
| ステップ7でユーザーが「HTML 見せて」「更新して」と言った | その時点の md 全体からフル再生成 → `SendUserFile` |
| ステップ8（終了時） | 最終版をフル再生成 → `SendUserFile` |
