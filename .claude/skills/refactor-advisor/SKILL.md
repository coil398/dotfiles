...
description: ローカルの差分・ファイルに、機能要件と別のrefactoring候補を提案する。reviewerの完了阻害判定と分け、保守性の改善余地を求められたときに使う。
...
`mkdir -p "${PROJECT_ROOT}/docs/reviews"` の上、`{RUN_DIR}/refactor-01.md` を `${PROJECT_ROOT}/docs/reviews/${run_ts}-${run_feature}.md` にコピーする（git 追跡対象。人が読む最終成果物）。`{RUN_DIR}/refactor-01.md` 自体は中間ファイルのまま `RUN_DIR` に残す。

> ℹ️ このスキルは **ユーザーゲート（all / 指定番号 / none 選択 → 適用）を持ちません**。提案を実装に反映したい場合は `/ir` や `/pir2` で「`{PROJECT_ROOT}/docs/reviews/{run_ts}-{run_feature}.md` の N 番を適用してほしい」と依頼してください。`/pir2` 経由なら適用後に再 reviewer で退行検知まで自動で回ります。
