---
name: deepthink
description: 特定の状況・論点を多エージェントで深く深く考え抜くワークフロー。オーケストレーター（スキル本体, Opus）が探索しまくり、複数の思考エージェント（deliberator）に多様なレンズで熟考させ、synthesizer が1本に統合し、gate が成功基準（rubric）に照らして客観的に十分性を判定する。満たすまで（必要なら追加探索を挟みつつ）ループし、gate が全基準の充足を客観的に確認できたら終了する。「じっくり考えたい」「深く考えて」「考え抜いて」「多角的に検討して」「結論を出したいが難しい」「意思決定を詰めたい」「〜すべきか徹底的に考えて」「腹落ちする答えがほしい」といった要望に対応する。単なる調査や仮説出し（それは /research）、コード実装・バグ修正・デバッグ（それは /pir2, /debug, /ir）ではなく、答えの出しにくい状況・論点をループで深掘りして客観的に十分な結論へ到達させたいときに使う。ユーザーが /deepthink と入力したら必ずこのスキルを使う。
...
深く考えるには「何をもって十分か」を先に決める必要があります。スキル本体が状況・論点を分解し、**この熟考が満たすべき成功基準（rubric）のドラフト**を `{RUN_DIR}/rubric.md` に Write します。
...
## ステップ 5.5: 未決事項の decision フェーズ（ユーザーに返す前に必ず通す）
...
最終サマリー（ステップ 6）を出す前に、position / gate レポートに以下のいずれかが含まれるとき:

- 「ユーザー承認待ち」「デフォルト案」「確定ではなく」
- 「実測が必要」「未検証」で、かつ**実行可能な検証手段が手元にある**もの
- 実装スコープ・実施順序が未確定
...
- 各決定に **決定 / 根拠 / 撤退条件（何が起きたら覆すか）の 3 点セット**を要求する
- 判断材料（コスト見積もり・既存資産への波及・測定行為が測定対象を壊す可能性 等）を渡す

決定は `{RUN_DIR}/decision-N.md` に書き出させる。

### ユーザーゲートを開いてよい例外

**不可逆・高リスクな操作のみ**（push / force-push / 破壊的削除 / 本番データ変更 / 外部への公開）。それ以外は decision フェーズで閉じる。

> ⚠️ **これは「聞かずに決めろ」という一般則の再掲ではない。** その一般則は既に各プロジェクトの SSOT にある。ここに書くのは **deepthink 固有の手順**（熟考の出口に decision フェーズを挟む構造）であり、二重管理を避けるため一般則そのものは書かない。
>
> **なぜ deepthink に書くのか**: 熟考の出口で「あとはユーザー判断です」と投げるのは、行き詰まりからの**非正規脱出**（責任の外部化）であり、熟考の成果を意思決定に変換しないまま終わらせる。ユーザーが `/deepthink` を起動した時点で「考えて決めてほしい」という要求は明示されている。
