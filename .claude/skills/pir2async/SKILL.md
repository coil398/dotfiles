...
## ステップ 5: 実装+レビュー チーム起動（1 implementer + 5 reviewer）

ここが通常の PIR² との違いです。implementer と 5体の reviewer（correctness / consistency / quality / security / architecture の5観点）を **Agent Teams** として起動し、直接対話させます。
...
description: "実装とレビューのチーム。implementerが実装し、5体のreviewer（correctness/consistency/quality/security/architecture）が並列で直接レビューしてフィードバックする。"
```

### 5-2: チームメイト起動（implementer 1体 + reviewer 5体 = 計6体）

以下の6つの Agent を**並列で**起動してください。全て `team_name: "impl-review"` を指定します。5体の reviewer は `REVIEWER_ROLE` と `name` だけが異なり、他は共通です。
...
```
あなたは impl-review チームの implementer です。チームには 5体の reviewer が並列で待機しています:
- reviewer-correctness (担当: バグ・正確性 / パフォーマンス / リグレッション)
- reviewer-consistency (担当: 命名規則・構造一貫性 / 同一ロジック全適用網羅性 / 類似ファイル群波及網羅性)
- reviewer-quality (担当: 保守性（局所スコープ）/ テストの質 / データアクセス重複 / スコープ逸脱)
- reviewer-security (担当: セキュリティ（OWASP）/ 認可・認証 / シークレット漏洩 / 依存脆弱性)
- reviewer-architecture (担当: レイヤリング / 循環依存 / 責務逸脱 / 抽象粒度)
...
2. 実装が完了したら、**5体の reviewer 全員に並列で SendMessage** して同時レビュー依頼する:
   - 送信先: reviewer-correctness / reviewer-consistency / reviewer-quality / reviewer-security / reviewer-architecture（5通を同時発火）
   - 本文（5通共通）:
     - 変更したファイル一覧
     - 実装内容の概要
     - `{RUN_DIR}/implementation-{IMPL_INDEX}.md` のパス
     - 「あなたの担当 role の観点でレビューお願いします」
3. 5体からそれぞれ VERDICT 返信（PASS / FAIL）を受け取る
4. VERDICT 集約:
   - **全体 PASS**: 5体すべて `VERDICT: PASS`
...
5. 全体 FAIL の場合:
   ...
   - `IMPL_INDEX` をインクリメントして新しい `{RUN_DIR}/implementation-{NN}.md` を書き出し、**5体全員に**再度レビュー依頼 SendMessage を送る（PASS を返した観点も再レビュー = 修正による退行検知のため）
6. 全体 PASS になるまで繰り返す
7. チームリードに SendMessage で以下を送る:
   ...
   - `{RUN_DIR}/review-{最終 REVIEW_INDEX}-{correctness|consistency|quality|security|architecture}.md` 5本の書き出し完了
   - 「実装+レビュー完了」

## 重要
- 5体の reviewer すべてと並列対話でレビューループを回すこと（逐次送信禁止、同時送信）
- チームリードへの報告は全体 PASS 後または打ち切り後のみ
- プラン外の変更は行わない
- 指摘事項の統合: 5体のレポートをマージ要約せず、各 `{RUN_DIR}/review-{NN}-{ROLE}.md` を直接 Read して修正すること（telephone-game effect 回避）
```
...
```
あなたは impl-review チームの reviewer-correctness です。チームメイトは implementer と、別観点の reviewer 4体（reviewer-consistency / reviewer-quality / reviewer-security / reviewer-architecture）です。各 reviewer は自分の担当観点のみを見ます。
...
2. ドキュメントの実装ログに追記する（5観点ごとの VERDICT も記載）
3. 6体すべてのチームメイトに shutdown_request を送る
4. ステップ 6 へ進む
