...
本スキルは pir2 スキルの references ファイル群を共有しています。以下のファイルを直接 Read します（pir2 を削除・移動・改名する場合は本スキル側のパス参照も同期更新が必要）:

- `~/.claude/skills/pir2/references/exploration-loop.md`
- `~/.claude/skills/pir2/references/refactor-advisor-gate.md`
- `~/.claude/skills/pir2/references/tester-prompt.md`
- `~/.claude/skills/pir2/references/continuation-gate.md`
- `~/.claude/skills/pir2/references/retrospector-prompt.md`
- `~/.claude/skills/pir2/references/handoff-cleanup.md`
- `~/.claude/skills/pir2/references/fan-out-gate.md`
- `~/.claude/skills/pir2/references/reviewer-set-algorithm.md`
...
- 最低1体起動。独立した複数領域に分割できるなら並列起動可（上限は CLAUDE.md の「コードベース探索の委譲」に従う）
- プロンプトに以下を含める:
...
チームメイト起動メッセージを送信する **直前のターン本文中** に、Fan-Out Gate 宣言テンプレートを必ず生成すること。テンプレート本体・運用ルール・違反パターンは `~/.claude/skills/pir2/references/fan-out-gate.md` を参照（Agent Teams 版テンプレートを使うこと）。OUTER_LOOP_COUNT 差し戻しでチームを再作成する際にも毎回この宣言を書くこと。
...
違反パターンは `~/.claude/skills/pir2/references/fan-out-gate.md` を参照（再起動する際も 5-2A からやり直す）。
...
REVIEWER_SET の観点と対応する reviewer name（担当分野は `~/.claude/skills/pir2/references/fan-out-gate.md` の「## 観点マッピング」セクションを参照）:
...
## ステップ 5.5: リファクタ提案（refactor-advisor 起動 → ゲート → 任意適用）

全体 VERDICT が PASS の場合のみ実行。チーム内ループ打ち切り（REVIEW_LOOP_COUNT >= 3）で FAIL 確定の場合はスキップしてステップ 6 へ。

詳細プロトコル: `~/.claude/skills/pir2/references/refactor-advisor-gate.md` を参照。

pir2async での適用ポイント:
- 5.5-1 ~ 5.5-4: 通常 pir2 と同じ（refactor-advisor 1 体起動、提案存在確認、ユーザーゲート提示、ユーザー選択処理）。`{RUN_DIR}/refactor-{最新 REVIEW_INDEX}.md` を書き出す
- 5.5-5: リファクタ適用時の implementer 再起動は **チームを再作成して** 行う（implementer + REVIEWER_SET reviewer の N+1 体、5-2A の Fan-Out Gate 宣言 → 5-2B の並列発火を再実行）。退行検知の再レビュー目的なので REVIEWER_SET は維持
- refactor-advisor は同一 run 内 **1 回のみ**起動（無限リファクタループ防止、pir2 と同じ）
- VERDICT 集約には影響しない（reviewer のみで判定済み）
