...
`mkdir -p "${PROJECT_ROOT}/docs/reviews"` の上、上記フォーマットで組み立てた統合レポート（対象〜主な指摘事項まで）を Write で `${PROJECT_ROOT}/docs/reviews/${run_ts}-${run_feature}.md` に保存する（git 追跡対象。人が読む最終成果物）。`{RUN_DIR}/diff.patch` および `{RUN_DIR}/review-01-{ROLE}.md` は中間ファイルのまま `RUN_DIR` に残す。
