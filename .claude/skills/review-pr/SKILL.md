...
2. **フラグ未指定時の自動選定**: 詳細プロトコル: `~/.claude/skills/pir2/references/reviewer-set-algorithm.md` を参照（デフォルト挙動 / ユーザーフラグ / 自動選定アルゴリズムの 5 ルール + フォールバック）。入力ソース: `gh pr diff <番号>` または `git diff <branch>...HEAD`、PR タイトル/本文を判定対象とする
