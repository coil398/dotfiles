...
`~/.cursor/projects/` 内で同一プロジェクトに対応する**複数ディレクトリが並存していないか**を機械的にチェックする。並存していると planner ステップ 1.5「既存ルール照合」や N4.4「既存ルール適用検証」が片方の系統しか参照せず、もう一方に蓄積された feedback が反映されない構造問題が発生する。

#### チェック手順

1. プロンプトで受け取った `PROJECT_MEMORY_DIR` から sanitized-cwd 部分（`~/.cursor/projects/<sanitized>/memory` の `<sanitized>` 部分）を抽出する
...
3. `ls ~/.cursor/projects/` でディレクトリ一覧を取得し、上記バリエーションのうち**現在の `PROJECT_MEMORY_DIR` 以外で存在するもの**を列挙する
...
同一プロジェクトに対応する複数のメモリディレクトリが `~/.cursor/projects/` 配下に並存しています。
...
N5 でエージェント定義を、または N6 で skill / agent を Edit/Write した場合のみ実行する。「改善したつもりが悪化」を無検証でコミットするのを防ぐ軽量ゲート。retrospector は `tools` に `Task` を持たない（制御フロー集約の原則に従い judge 系のネスト起動は設計上許可しない）ため、別 judge エージェントをネスト起動しない。よって**実際に過去タスクを再実行するのではなく、コミット前の現行版（before）と編集後（after）を自分自身で批判的に読み比べる自己 A/B judge** を行う（reasoning 起動時はより厳密になる）。
...
sweep_cur=$(grep -hc '^- 使用モデル:' "${HOME}"/.cursor/projects/*/memory/pir_skill_log.md 2>/dev/null | awk '{s+=$1} END{print s+0}')
