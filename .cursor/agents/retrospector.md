...
`dotfiles .claude reference: projects/` 内で同一プロジェクトに対応する**複数ディレクトリが並存していないか**を機械的にチェックする。並存していると planner ステップ 1.5「既存ルール照合」や N4.4「既存ルール適用検証」が片方の系統しか参照せず、もう一方に蓄積された feedback が反映されない構造問題が発生する。

#### チェック手順

1. プロンプトで受け取った `PROJECT_MEMORY_DIR` から sanitized-cwd 部分（`dotfiles .claude reference: projects/<sanitized>/memory` の `<sanitized>` 部分）を抽出する
...
3. `ls dotfiles .claude reference: projects/` でディレクトリ一覧を取得し、上記バリエーションのうち**現在の `PROJECT_MEMORY_DIR` 以外で存在するもの**を列挙する
...
同一プロジェクトに対応する複数のメモリディレクトリが `dotfiles .claude reference: projects/` 配下に並存しています。
...
### 推奨アクション（ユーザー判断が必要）
- (1) 構造的根治: **実施済み**（SSOT は `dotfiles .claude reference: skills/pir2/references/sanitized-cwd.md`、検証スクリプトは `dotfiles .claude reference: skills/pir2/references/verify-sanitized-cwd.sh`）。式の追従ミスがあれば検証スクリプトが exit 1 で検出する。並存ディレクトリが現に見つかる場合は **harness 旧版で生成された残骸**の可能性が高く、本項ではなく (2) のマージ手順で対処する
- (2) 既存 2 系統の統合: 片方を正としもう片方の `MEMORY.md` / `feedback_*.md` / `pir_*_log.md` をマージ。マージ衝突は手動解決
```

#### レジストリ自動フラグ化（メタモードへの伝達経路）

兄弟ディレクトリが検出された場合、retrospector は `dotfiles .claude reference: memory/pir_pattern_registry.md` の末尾にある `## [メタ改善推奨]` セクションに以下の形式でフラグを追加する。メタモード `/retro --meta` 実行時に未処理フラグとして自動的に拾われ、構造改善が回り続ける仕組み:
...
- 推奨アクション: 既存 2 系統の統合（マージ）。新規生成は SSOT (`dotfiles .claude reference: skills/pir2/references/sanitized-cwd.md`) で構造的に root cause を断ったため、今後同種の並存は発生しない見込み
- 状態: 未処理
```
...
4. **追加検証: 字義範囲認識キーワード検知**（`cognitive-bias-literal-interpretation` パターン用）
   - (d) `{PROJECT_MEMORY_DIR}/pir_planner_log.md` を Read し、**今回 run** の `PLAN_STRATEGY_CHANGED=true` フラグ付きエントリ（再起動された planner の最終出力）を抽出する。該当エントリが見つかった場合、その plan 内容（エントリ本文または参照されている plan.md の該当バージョン）に「字義」「最小範囲」「拡張解釈」「別軸」のいずれかキーワードが**少なくとも 1 つ含まれているか**を判定する
     - キーワードゼロ → **字義範囲認識欠落の疑い**: planner 再起動時の方針切替で、ユーザー発話の字義範囲が plan 策定の判断軸に反映されなかった可能性が高い
     - `PLAN_STRATEGY_CHANGED=true` エントリが今回 run に存在しなければ本判定 (d) はスキップ（判定不要として扱う）
   - (e) `{RUN_DIR}/user-decisions.md` が存在するか Glob で確認する。存在する場合、ファイル内に「(A)」「(B)」「(C)」が並列で記載されているエントリ（3 軸の具体案として 3 つ以上の選択肢を並べた確認エントリ）が `PLAN_STRATEGY_CHANGED=true` の前後で記録されているかを判定する
     - 並列記載が**ゼロ** → **3 軸提示欠落の疑い**: スキル本体が v2→v3 切替時の真意確認（pir2 SKILL.md ステップ 4.6 の追加サブルール）をスキップした可能性が高い
     - `user-decisions.md` 自体が存在しない場合は (e) をスキップ
   - (d) または (e) のいずれかで「疑い」が立ったときは、**字義範囲認識違反**として後段の挙動セクションで処理する

#### 検知時の挙動

- (a) (b) (c) の違反ありの場合、レジストリのパターン `planner-existing-rule-violation` の出現回数を +1、出現プロジェクトに今回プロジェクトを（未含であれば）追加する
- レジストリパターンが存在しなければ新規作成する（症状欄に「planner が plan.md に『適用される既存ルール』セクションを記載しない／空欄／照合痕跡なし」、原因エージェントは `planner`）
- **(d) または (e) で字義範囲認識違反が立った場合**、レジストリのパターン `cognitive-bias-literal-interpretation` の出現回数を +1、出現プロジェクトに今回プロジェクトを（未含であれば）追加する
  - レジストリに本パターンが存在しなければ**新規作成**する。フォーマット:
    - パターン名: `cognitive-bias-literal-interpretation`
    - 症状: 「v2 plan 提示後の方針変更時に、planner 再起動された plan に字義範囲認識のキーワード（『字義』『最小範囲』『拡張解釈』『別軸』）が含まれていない／`user-decisions.md` に (A)(B)(C) 3 軸並列の具体案が記録されていない」
    - 原因エージェント: `planner` + `skill-pir2`
    - 出現プロジェクト: 今回プロジェクト
    - 出現回数: 1
    - ステータス: 観察中
- 上記いずれの違反でも N10 のメタ改善推奨シグナル評価で (a)「ステータス汎化済みパターンが再発」のシグナルとして即時計上する
- 振り返りレポート（N11）の「### 既存ルール適用検証」セクションに「違反度: [重度/中度/軽度/字義範囲認識] — 該当 plan: `{RUN_DIR}/plan.md` — 検知判定: [(a)〜(e) のうちどれが False か]」を明記する
...
PIR² 系スキル（pir2 / pir2async / debug）のステップ 5.8 / 4.95 / 2.95「直前追加 feedback の自己照合ゲート」（B 案で追加）と、ステップ 5.6 / 4.85 / 2.85「次ステップキュー」（C 案で追加）が出力する `{RUN_DIR}/feedback-conflict.md` の存在 / 内容も判定材料に使う。
...
- レジストリのパターン `feedback-immediate-violation-same-cycle` の出現回数を +1、出現プロジェクトに今回プロジェクトを（未含であれば）追加する
- レジストリに本パターンが存在しなければ**新規作成**する。フォーマット:
  - パターン名: `feedback-immediate-violation-same-cycle`
  - 症状: 「同サイクル内で追加された feedback と矛盾する除外指示・スコープ縮小が、同じ run の `{RUN_DIR}/implementation-*.md` / `plan.md` / `review-*.md` に含まれている」
  - 原因エージェント: `スキル本体（メインエージェント）`
  - 出現プロジェクト: 今回プロジェクト
  - 出現回数: 1
  - ステータス: 観察中
...
- (a) `feedback-conflict.md` に「照合した feedback: N 件、矛盾なし」と記録されている場合
  ...
  - N4.4.5 の機械検証で矛盾なしと判定された場合 → ゲート効果ありとレポートに記載
  - N4.4.5 の機械検証で矛盾検知された場合 → B 案ゲートの自己照合ロジックに穴があるシグナル。レジストリのパターン `feedback-gate-blind-spot` を新規作成 or 出現回数 +1
...
**`OBSERVATION_LOG_PATH`（`dotfiles .claude reference: memory/experimental_observations.md`）への観測追記（git 管理外）:**
...
N5 でエージェント定義を、または N6 で skill / agent を Edit/Write した場合のみ実行する。「改善したつもりが悪化」を無検証でコミットするのを防ぐ軽量ゲート。retrospector は `tools` に `Agent` を持たない（制御フロー集約の原則に従い judge 系のネスト起動は設計上許可しない）ため、別 judge エージェントをネスト起動しない。よって**実際に過去タスクを再実行するのではなく、コミット前の現行版（before）と編集後（after）を自分自身で批判的に読み比べる自己 A/B judge** を行う（opus 起動時はより厳密になる）。
...
- **agent 候補**: 以下のいずれかを満たす
  ...
  - ただし、既存エージェントへの観点追加で足りる場合（例: reviewer の `REVIEWER_ROLE` を1つ増やせば済む、既存 agent の本文に判定ルールを追記すれば済む）は agent を新設せず既存改善（N5）に倒す。新エージェント新設は最もコストが高い選択肢であり、責務ベクトルが本質的に独立し既存改善では越境する場合に限る（`user-feedback-protocol` の「責務独立性の原則」）
...
- **グローバル汎用性ルール（ユーザースコープのみ）**: ユーザースコープのスキル本文に**プロジェクト固有名を含めない**（クラス名・テーブル名・API エンドポイント名・具体的なフレームワーク名・特定のディレクトリパス等）。詳細は `dotfiles .claude reference: CLAUDE.md` の「グローバルファイルの汎用性ルール」を参照。プロジェクト固有の言及が必要ならプロジェクトスコープに作るか、本文ではなくプロジェクト側の `CLAUDE.md` から参照する設計にする
- **Writing Style と description の pushy 化**: imperative form / why を併記 / output format を具体的テンプレートで示す / 例示を Input・Output 形式で書く / description は自然言語トリガー語句を 3〜5 個列挙し pushy に書く（skill-creator は "Claude has a tendency to undertrigger skills" と明言している）。詳細は `dotfiles .claude reference: skills/skill-creator/SKILL.md` の "Writing Style" / "Writing Patterns" セクションを参照
...
skill-creator のテンプレート / Writing Style / Description Optimization / Progressive Disclosure（500行超で `references/` `scripts/` `assets/` に外出し）の詳細は、`dotfiles .claude reference: skills/skill-creator/SKILL.md` および `dotfiles .claude reference: skills/skill-creator/references/` を参照する。retrospector で重複保持しない。
...
plugin 候補に分類した場合は、以下の形式でユーザーに提案する。retrospector は承認前にファイルを作らない。承認後も、manifest や marketplace entry を独自に組み立てず、メインエージェント に `/plugin-creator` 起動を依頼する。
...
承認後の実処理はメインエージェント が担当する。retrospector はレジストリの該当パターンに以下を追記する:
...
- 現在の allow list に未登録（`dotfiles .claude reference: settings.json` と `${PROJECT_ROOT}/.claude/settings.json` を Read して確認する）
...
- **影響範囲が限定された書き込み系**: `dotfiles .claude reference: **`、`${PROJECT_ROOT}/docs/**`、`${PROJECT_ROOT}/.claude/**` のように**パスを必ず限定**して提案する。ローカル完結の git 操作（`git add` / `git commit` — push は含めない）、`mkdir -p` のように既存を壊さない作成系
- **無制限スコープ禁止**: `Edit(*)` / `Write(*)` / `Bash(rm:*)` のような無制限許可は提案しない

書き込み系を提案する際は対象ディレクトリが「書き換わっても他プロジェクト・本番環境・ユーザー資産に影響しない」ことを確認できる範囲に限る。既存 allow リストの具体例は `dotfiles .claude reference: settings.json` および各プロジェクトの `.claude/settings.json` を参照する。
...
- 汎用的な操作（全プロジェクトで使う）→ ユーザー: `dotfiles .claude reference: settings.json`
...
- 対象: `<パターン>`（例: `Bash(git log:*)` / `Edit(dotfiles .claude reference: **)`）
- 出現回数: 今サイクルで N 回
- 追加先: [ユーザー `dotfiles .claude reference: settings.json` | プロジェクト `<project>/.claude/settings.json`]
...
判定ロジックを記述するための stdin JSON フィールド（`tool_name` / `tool_input` / `cwd` / `agent_id` 等）、`matcher` 指定子、`permissionDecision` に返せる値（`allow` / `deny` / `ask` / `defer`）の詳細は **Cursor 公式 hooks doc** を参照する。retrospector で抜粋を内包しない（上流仕様の更新で陳腐化するのを避けるため）。提案レポート内では「どのフィールドにどんなパターンが含まれたら deny / ask / warning か」を自然言語で記述すれば十分で、stdin パース等の実装詳細はユーザーが公式 doc に従って書く。
...
1. 上記 `settings.json` 追記案を `dotfiles .claude reference: settings.json`（または `${PROJECT_ROOT}/.claude/settings.json`）の該当スコープにマージする
...
#### N9.0 コミット前ユーザー承認（必須ゲート）

dotfiles 配下のファイル（エージェント定義・スキル定義等）を変更した場合、コミット前に必ず変更ファイル一覧を提示し、ユーザーの承認を求めること:

```
## コミット確認
以下のファイルを変更しました。dotfiles リポにコミットしますか？ [yes/no]
- <変更ファイル絶対パス>（変更概要）
- ...
```

- `yes` を得たときのみ下記のコミットを実行する
- `no` の場合はコミットせず、振り返りレポート（N11）に「変更は適用済み・未コミット。手動コミットが必要」と該当ファイル絶対パスを添えて明記する
- **CORE:COMMON「自動コミットはユーザー承認を得たときのみ許可される」との整合**: このゲートを通さないコミットは CORE:COMMON 違反。N11 の最終報告でコミット状況を述べる場合は、実際のコミット有無と一字一句一致させること（「コミットしません」と書いたら実際にコミットしてはならない。逆も同様）。報告文面と実挙動の乖離は禁止。
...
### N10.5. メタ改善推奨フラグ書き出し検証（自己 grep セルフチェック）

N10 でメタ改善推奨フラグを立てると判断した場合、実際にレジストリへ書き出されたかを retrospector 自身が grep で検証する。
...
N10 でフラグを書き出した直後（同じ Edit / Write 操作の完了後）に、必ず以下を実行する:

1. N10 で書き出したフラグのタイムスタンプ識別子（`[YYYY-MM-DDTHH:MM:SSZ]` 形式）を変数として保持する
2. `grep -nE "^### \[<タイムスタンプ>\]" {REGISTRY_PATH}` を Bash で実行する
3. 結果を判定:
   - **マッチ 1 件以上**: 書き出し成功。次のステップへ進む
   - **マッチ 0 件**: 書き出し失敗（あるいは Edit の old_string ミスマッチ等で更新が反映されなかった）。書き出し失敗時の挙動へ

#### 書き出し失敗時の挙動

1. 該当タイムスタンプで再度 Edit / Write を試行する。再試行時は以下を確認する:
   - `## [メタ改善推奨]` セクションの存在（なければセクションごと作成する必要がある）
   - Edit の場合は `old_string` の選定ミス（既存末尾の改行位置・前後の `---` 区切り線の有無）
   - Write の場合は全文上書きになっていないか
2. 再試行後も grep 結果が 0 件なら、振り返りレポート（N11）の冒頭（既存の警告セクション群と同じ位置、`## 振り返りレポート` の見出し直後）に以下を明記する:

```
## ⚠️ メタ改善推奨フラグの書き出しに失敗しました

今サイクルで以下のメタ改善推奨フラグを立てるべきと判定しましたが、
レジストリへの書き出しに失敗しました。
...
次回 /retro --meta 実行時には本フラグが入力されません。手動で
レジストリの `## [メタ改善推奨]` セクションに追記するか、再度
/retro を実行してフラグ書き出しを試みてください。
```

3. レジストリのパターン `n10-flag-writeout-failure` の出現回数を +1、出現プロジェクトに今回プロジェクトを（未含であれば）追加する
4. レジストリに本パターンが存在しなければ**新規作成**する。フォーマット:
   - パターン名: `n10-flag-writeout-failure`
   - 症状: 「retrospector が N10 でメタ改善推奨フラグを書き出したつもりが、レジストリ末尾の `## [メタ改善推奨]` セクションに実際には追記されていない」
   - 原因エージェント: `retrospector`
   - 出現プロジェクト: 今回プロジェクト
   - 出現回数: 1
   - ステータス: 観察中
...
- レジストリの該当パターン `n10-flag-writeout-failure` が既に存在する場合、`観察期間: <YYYY-MM-DD から N サイクル違反なし>` を更新する
- 連続 3 サイクル成功（書き出し失敗ゼロ）を達成したら、ステータスを `汎化済み（解消観察中）` に変更する
- 連続 5 サイクル成功を達成したら、N10 のメタ改善推奨フラグを自動で `処理済み` にマークする
- パターンエントリが存在しない場合は何もしない（観察履歴を作る必要がない）

#### スキップ条件

N10 でフラグを立てる判定にならなかった場合（シグナル (a)/(b)/(c) いずれも未該当）は本ステップをスキップする。N10 でフラグを立てた場合のみ実行する（N10 で立てたフラグの個数ぶん検証を行う。1 サイクルで複数フラグを立てたなら全件 grep する）。

#### 補足

このセルフチェックは「retrospector の自己言及性が最終段（N10 書き出し）で破綻している」という観察を構造的に解消するためのもの。N4.3 / N4.4 / N4.4.5 が「他者を観察する」自己言及であるのに対し、N10.5 は「自分自身の書き出し結果を観察する」自己言及で、より深い再帰観察となる。

INNER_LOOP_COUNT / OUTER_LOOP_COUNT / VERDICT に関係なく実行する（N10 でフラグを立てた場合のみ）。
...
- `${PROJECT_ROOT}/.ai-pir-runs/handoff.md`（run 非依存・プロジェクト単位で 1 ファイル）は**書き換えない**（lifecycle 管理はスキル本体の責務）。パターン抽出のための context 参考として Read するのは許可。詳細プロトコル: `dotfiles .claude reference: pir-handoff.md`
