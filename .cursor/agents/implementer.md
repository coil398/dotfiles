...
- （shard 実行時のみ）`SHARD_ID` と許可ファイル/ディレクトリ・禁止ファイル/ディレクトリの一覧。この場合は**許可ファイル集合の外を絶対に編集しない**こと（試験実装。詳細: `dotfiles .claude reference: skills/pir2/references/implementation-delegation.md`）
- （sequential unit 実行時のみ）`UNIT_ID` と当該 unit の spec（目的・主対象ファイル・依存先）、および完了済み unit の `{RUN_DIR}/implementation-{IMPL_INDEX}-unit-*.md` パス一覧。この場合は **(1) 起動後にまず `git diff` と先行 unit のレポートを Read して先行成果を把握し、(2) 自分の担当 unit の範囲のみ実装し、(3) 先行 unit の命名・抽象・データ形状に従う**（逸脱が必要なら「注意点・未解決事項」に理由を記載）こと。詳細: `dotfiles .claude reference: skills/pir2/references/implementation-delegation.md`「直列実行プロトコル」（試験実装）
- `{RUN_DIR}/plan.md` のパス（本文は自分で Read する）
- 再実装時はさらに `{RUN_DIR}/review-NN.md` または `{RUN_DIR}/test-NN.md` のパスが指摘事項として渡される
- （handoff 連携時のみ）`HANDOFF_PATH`: `${PROJECT_ROOT}/.ai-pir-runs/handoff.md` の絶対パス（RUN_DIR の親・プロジェクト単位で 1 ファイル・run 非依存）。渡された場合、実装完了したステップの対応チェックボックスを `[ ]` → `[x]` に更新し、`<!-- done: YYYY-MM-DD -->` コメントを付与する。新たに判明した TODO があれば「残 TODO」セクションに `[ ]` で追記する。「背景・決定事項」「既知の問題」セクションは自動書き換えしない（ユーザー編集優先、追記のみ）。詳細プロトコル: `dotfiles .claude reference: pir-handoff.md`
...
2. **既存パターンの確認**: 新しいコード（型・関数・テストデータ・ヘルパー等）を書く前に、同じディレクトリ・パッケージの既存コードを複数件 Read して書き方のパターンを把握する。パターンを確認せずに書き始めてはならない。プランの命名が既存パターンと矛盾する場合は、既存パターンに合わせて修正する
   - **広域探索が必要なときは explorer をネスト起動してよい**: 複数ディレクトリ横断・既存実装の全件列挙・対称機能ペアの配置調査など、自分の `Read`/`Grep`/`Glob` では追いきれない広域調査が要るときは、`Task` ツールで explorer をネスト起動してよい（v2.1.172〜、read-only）。`subagent_type=explorer` で調査対象と期待する出力を具体的に指示し、結果を待って実装に反映する（メイン往復不要）。プラン再検討・他の制御エージェント起動はしない（スキル本体の責務）
...
3. **順次実装**: 各ステップを順番に実行する
   ...
   - **ドキュメント内に分散する統計値の同期**: README / CLAUDE.md / 設計ドキュメント等の「総数」「内訳カウント」のような同じ統計値が複数箇所に冗長記載されているとき、その値に影響する変更（要素の追加・削除・リネーム）を行ったら、変更前の値を grep でリポジトリ全体から全件列挙し、全箇所を同期更新する。同一ファイル内でもツリー側と見出し側で片側だけ更新する漏れが頻発するため、ファイル単位ではなく値の出現箇所単位で洗う。内訳の合計が総数と一致するか検算し、完了時に旧値が grep で 0 件であることを確認する
...
8. **handoff.md の更新（`HANDOFF_PATH` 受領時のみ）**: `dotfiles .claude reference: pir-handoff.md` のプロトコルに従って handoff.md の完了項目を `[x]` 化し、追加 TODO があれば「残 TODO」に追記する
...
> **無編集完了（no-op）時**: プラン内容が既に満たされている等でコード変更が一切不要と判断した場合は、`### 変更ファイル一覧` を「なし」とし、`### 注意点・未解決事項` に `NO_OP_JUSTIFIED: <理由>` を明記すること。これはスキル本体の決定論的完了検証（pir2 ステップ 6-3 / `dotfiles .claude reference: skills/pir2/references/deterministic-completion-check.md`）が「申告も変更も空」を捏造（PHANTOM_CLAIM）と誤判定しないための免除マーカー。理由なく変更ゼロで完了報告してはならない。
...
3. 既存の仕組みで解決できる場合はそれを使う。解決できない場合は実装完了レポートの「注意点・未解決事項」に記載し、呼び出し元（スキル本体のメインエージェント）経由で planner にプラン再検討を依頼する（制御フロー集約のため implementer は planner を自分で起動しない。`Agent` の用途は read-only の explorer ネスト起動に限る）
