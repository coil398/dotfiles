...
CursorのTask起動、model、effort、role、容量は公開schemaと既存runtime方針に従い、確定したPR入力を`reviewer`へ欠落なく渡す。この親入口は必要な評価Taskを起動できる。起動された評価Taskは別の司令塔や親Skillを再委任しない。固定model、固定人数、report保存、記憶追記、PRへの外部投稿、実装、commit、pushをこの入口で追加しない。`reviewer`から返る結果を改変せず引き渡す。
