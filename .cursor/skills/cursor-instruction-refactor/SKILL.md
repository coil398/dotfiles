...
このスキル本体（= メインエージェント）がオーケストレーターとなり、`explorer` を `Task` ツールで起動して測定・分析を行い、改善は本体が直接 Edit/Write で実施します。
...
スコープのデフォルトはユーザースコープ（`~/.cursor/` 配下）。プロジェクト固有の `.codex/` 配下を含めたい場合は `--scope=project` または `--scope=all` を指定する。短縮形 `--user` / `--project` / `--all` も `--scope=` と等価。位置引数のパスは **1 つだけ**対象になる（2 つ目以降は警告して無視）。

---

## ステップ 2: 対象ファイルの探索と測定（公式定量基準の照合）

`explorer` エージェントを `Task` ツールで起動して測定を委譲してください:

- **model**: role=coding（広く浅い列挙が中心）
- **プロンプトに含めるパラメータ**:
  - `SCOPE`（user / project / all）
  - `TARGET_PATH`（指定があれば、ない場合はスコープ全体）
  - 「以下を測定してレポートを返してください:
    1. SCOPE に応じた対象ファイルを Glob で列挙する
       - `user`: `AGENTS.md (shared SSOT)`, `~/.cursor/agents/*.md`, `.cursor/skills/**/SKILL.md`
       - `project`: `${PWD}/.codex/AGENTS.md`, `${PWD}/.codex/agents/**/*.md`, `${PWD}/.agents/skills/**/SKILL.md`
       - `all`: 両方
    ...
    6. 計測結果を表形式で返す」
  - 「判断基準は `.cursor/skills/cursor-instruction-refactor/references/official-criteria.md` を Read して使うこと」
...
- **プロンプトに含めるパラメータ**:
  ...
  - 「`.cursor/skills/cursor-instruction-refactor/references/checklist.md` の判定 2（責務越境 / SSOT 逸脱 / DRY 違反 / 二重説明）と判定 3（description 適切性）と判定 4（グローバル汎用性）に従い、各ファイルの構造的悪さを検出してください。検出した場合は『該当箇所の行範囲』『種別』『理由』『推奨整理戦略』を報告してください」
  - 「整理戦略の詳細は `.cursor/skills/cursor-instruction-refactor/references/strategies.md` を参照すること」
  - SCOPE に応じた SSOT ファイル一覧を渡す（例: `/skill-creator` の SKILL.md / `~/.cursor/agents/reviewer.md` / `AGENTS.md (shared SSOT)` 等。判定 2b の SSOT 逸脱検出に使う）

DRY 違反を疑う場合は、対象ファイル群を pairwise で比較して連続 5 行以上の重複を検出するよう指示する。

**判定 4（グローバル汎用性）は構造読解の副産物にしない**。explorer に「対象ファイル**全件**への独立した固有名 grep スイープを行い、候補語を `~/.cursor/history.jsonl` / `~/.cursor/memories/*/memory/` と cross-reference して generic な一般ツールと project-specific leak を実プロジェクト照合で区別する」よう明示する（checklist.md 判定 4 の 2 段階手順）。`make <固有ターゲット>` / `[a-z]+_id` / 具体 ORM 名 / ドメイン固有名詞 を見落とさないこと。**リファクタ適用後は候補語パターンを全件へ再 grep し残存ゼロを機械確認する**（部分スイープの取りこぼし防止）。
...
`all` または番号指定の場合、メインエージェント が直接 Edit/Write で修正する。
