...
親Cursor agentがscope、所有、受入、結果統合を持つ。小さく密結合した変更は直接実装でき、分離価値がある場合だけ標準implementer Taskへ、目的、確認済み事実、排他的所有、禁止範囲、完了条件、focused checkを渡す。Taskのmodelは省略または`inherit`とし、子へ親用IR工程や別Taskを再起動させない。
