...
スキル終了後もユーザーは通常チャットでメインエージェント に質問し続けられる。メインエージェント は最後に作成/更新したウォークスルーファイルをコンテキストに保持しているためそのまま Q&A に応答できる。再度スキル起動したい場合はユーザーが `/walkthrough` を呼び直す。

---

## 既存スキルとの棲み分け

- `/pir2` のサマリー版ウォークスルー: 自分が走らせた実装直後の1発ウォークスルー。スキル本体（メインエージェント）が `/pir2` ステップ9で生成する
