...
- 独立したサブ問いがあり分離の利益がある場合だけ、資料探索は`explorer` Taskを起動します（共有 `references/explorer.md` の実体pathを `SKILL_PATH` として渡す）。対象版、問い、確定事実、編集禁止、根拠をチャットで返すことと、必要な実行者用 Skill / reference の実体 path を渡し、子自身に資料を Read させます。
- 技術選定が必要な場合は、Taskへ共通原本から解決した `references/tech-validator.md` の実体pathと比較条件を渡すか、親が公式資料を比較します。委任時は子がその reference をReadし、親が直接比較する場合だけ親がReadします。役名や固定人数を完了条件にしません。
- readerは常に親へチャットで返し、保存、記憶追記、コード変更、テスト生成、外部投稿をしません。Taskの失敗・timeout・未読・取得不能は未確認と明示します。子は親用 `/research` の再委任・再集約手順を再起動しません。

## 思考・仮説

思考・仮説形成を委任する場合は、対応する共有referenceの実体path、実在する入力、対象版、編集禁止、チャット返却をTaskへ渡し、担当自身にreferenceをReadさせます。親が直接集約・分析・仮説形成する場合だけ、必要なreferenceを親がReadします。特定のTask識別子や固定人数を要求せず、thinker相当は新規調査をせず、hypothesizer相当は検証方法の設計までです。
