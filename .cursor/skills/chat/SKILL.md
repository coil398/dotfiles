...
- ローカルコードは親の直接確認、または `explorer` Task に対象版、範囲、問い、編集禁止、チャット返却と必要な実行者用 Skill / reference の実体 path を渡して確認します。委任時は子自身が資料をReadし、親が直接確認する場合だけ親が必要なreferenceをReadします。readerに保存、記憶追記、コード変更、外部投稿をさせません。
