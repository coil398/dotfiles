...
ユーザーが `/chat <問い>` と入力したらこのスキルが起動する。普段の Codex の応答とは別モードに切り替わり、**「裏取りせずに端的に答える」「一般論で済ませる」「気が利かない」を明示的に禁止**する。
...
- [ ] **Codex の設定**: `~/.cursor/` 配下の skills, agents, settings, MCP servers
...
> ℹ️ Codex のsubagent制約: subagentは Task subagentを呼べない。`/chat` 起動時のメインエージェント（このスキルを実行している主体）からのみ委譲できる。
...
通常の Codex セッションでは「軽い質問だから直接答える」が許容される場面でも、`/chat` 起動時はそれを禁止する。`/chat` を打つ = 「手抜きせず深掘りしてほしい」というユーザーからの明示的な要求。
