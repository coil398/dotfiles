...
- 「ユーザーの dotfiles は ~/dotfiles にある」「Linux 環境」「Cursor 利用」など、コンテキストにある事実は前提として組み込む
- 「あなたの環境では」と明示する
...
- [ ] **Cursor の設定**: `~/.cursor/` 配下の skills, agents, settings, MCP servers
...
> ℹ️ Cursor の委譲手順: `/chat` 起動時のメインエージェント（このスキルを実行している主体）が `Task` による委譲の起点と結果統合を担う。
...
`AGENTS.md (shared rule)` には既に「コードベース探索は explorer に委譲」「ライブラリ選定は tech-validator」のルールがある。`/chat` はそれらを**強化**するスキルで、雑談モード／ナレッジ問い合わせモードでも同じレベルの裏取りと深掘りを保証する。

通常の Cursor セッションでは「軽い質問だから直接答える」が許容される場面でも、`/chat` 起動時はそれを禁止する。`/chat` を打つ = 「手抜きせず深掘りしてほしい」というユーザーからの明示的な要求。
