...
```
依頼モードを選ぶ
  作成 → BOOTSTRAP.md: プロジェクトのコンテキストからSSOTを生成
  評価 → AUDIT.md: 現状とあるべき姿のgapを検出し、指摘・未確認を返す
  反映 → SSOTを更新し、コードへ反映してAUDIT.mdで再評価
```

`評価`（`audit-only`）ではSSOTがなくても評価可能な項目を続け、不在・未確認を報告する。SSOTの生成やAUDITの修正手順は、作成・反映の依頼または既存の明示承認がある場合だけ実行する。
