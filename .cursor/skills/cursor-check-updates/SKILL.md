...
1. **マーケットプレースプラグイン** — `~/.cursor/plugins/marketplaces/` 配下
2. **インストール済みプラグイン** — `~/.cursor/plugins/cache/` 配下
3. **ユーザースコープ skills** — `.cursor/skills/` 配下の git リポジトリ
...
```bash
sh "$(dirname "$(readlink -f .cursor/skills/cursor-check-updates/SKILL.md)" 2>/dev/null || echo ".cursor/skills/cursor-check-updates")/scripts/check-updates.sh" "$(pwd)"
```
