...
6. review/test/再確認、権限確認、破壊的変更の独立検証はpir2のリスク相応ルールをそのまま適用します。評価を起動する場合は `${CURSOR_SKILLS_DIR}/code-review-guidance/references/result-contract.md` の `COVERAGE` / `VERDICT` を保持し、未確認をPASSへ変換しません。縮退を理由に品質・安全境界を弱めません。
