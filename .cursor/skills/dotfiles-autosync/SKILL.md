...
Cursor の親が対象 root、既存 upstream、commit/merge/generator/push の実行と結果統合を持つ。engine を起動する場合は共有原本の実体から解決した engine path と明示 root を使い、Task の model/effort は AGENTS の runtime 方針に従ってこの入口では固定しない。read-only 担当を使う場合も、親が確認した path と返却形式を渡し、子に親の同期工程を再起動させない。
