...
description: >-
  ...
  「lesson」「recall field notes」「field-notes triage」「/cursor-field-notes」。
  MEMORY/LTMの代替ではない。日記（/cursor-ai-diary）・横断検索（/cursor-ai-ltm）とは別。
...
- 毎ターン・毎コマンド成功ごと
- 単なる進捗報告（「r05 終わった」だけ）
- 感想・日記・横断検索が欲しいとき → `/cursor-ai-diary` / `/cursor-ai-ltm`

プロジェクトに `CLAUDE.md` の置き場ルールがあればそれに従う。無ければユーザー確認後に `~/cursor-field-notes/`（勝手に mkdir しない）。
...
| `/cursor-ai-ltm` | 過去の経緯の検索倉庫 | 必要時検索（自動: 下表） |
| references / SSOT | 一般化済みの正式仕様 | 該当 role 実行時 |
| `/cursor-ai-diary` | 感想・その日の物語 | 記録用（技術判断に使わない） |
...
```text
~/cursor-field-notes/<campaign-or-topic>/
  INDEX.md
  YYYY-MM-DD-<short-slug>.md
```
...
- 秘密（`.env`）を書かない
- field-notes を検索基盤化しない（それは `/cursor-ai-ltm`）
