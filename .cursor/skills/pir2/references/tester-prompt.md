...
PIR² 系スキル共通の tester 実行仕様。スキル本体（メインエージェント）は tester subagent を起動する、またはメインエージェント が直接テストする **前に** TEST_SCOPE を構築して明示する。
...
subagent が利用可能なら `tester` を起動する。利用できない場合はメインエージェント が同じプロンプト項目に従ってテストを実行し、同じ `{RUN_DIR}/test-{TEST_INDEX}.md` を書き出す:
