...
ステップ 12 では、実際に確認できた差分・コマンド結果・shared reviewer / tester の報告だけを使ってユーザーへ提示する。実行していない工程、存在しない report、固定のループ回数や担当人数を補完しない。該当しない項目は省略または「未実施」とし、未確認の重大リスクを成功と扱わない。結果の語彙と集約は、親が実体を確認した各 shared skill の契約に従う。
...
### レビュー結果
- shared reviewer の実際の返却: [実行した確認、結果、未確認範囲]
...
### テスト結果
- shared tester の実際の返却: [実行した確認、結果、未確認範囲。未実施なら理由]
