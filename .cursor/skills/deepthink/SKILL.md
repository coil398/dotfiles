...
> - **本スキル例外**: deliberator / synthesizer / gate だけはユーザー指定どおり `claude-fable-5-1[effort=high]` を Task `model` に渡す（短名 `fable` は使わない。SSOT: `.agents/skills/deepthink/references/fable-model.md`）。explorer は `inherit` のまま。Fable 起動失敗時は inherit + reasoning-panel にフォールバック
...
| フェーズ | 担当 | Task `model` |
|---------|------|------|
| 探索 | explorer（最大4体並列） | `inherit`（または高速寄りの利用可能 ID） |
| 集約 + rubric 確定 | オーケストレーター（スキル本体） | 親セッション |
| 熟考 | deliberator（**1体**） | `claude-fable-5-1[effort=high]` |
| 統合 | synthesizer | `claude-fable-5-1[effort=high]` |
| ゲート（十分性判定） | gate | `claude-fable-5-1[effort=high]` |
...
- `$ARGUMENTS` から `--opus-panel` / `--panel` / `panel` / `--effort=max` / `--effort=high` を検出し、フラグ語はタスク文言から除外する。
- effort 未指定時は `high`。Fable 起動がプラン制限等で失敗したら `reasoning-panel` にフォールバックしサマリーに記録する。
