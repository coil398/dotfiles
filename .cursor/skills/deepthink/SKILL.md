...
実在する入力だけで判断できるなら探索を省略します。不足があり独立した調査に価値がある場合だけ、`explorer` Taskへ具体的な問いを渡します。担当には対象版、範囲、確定事実、調査観点、編集禁止、返却事項と、必要な実行者用 Skill / reference の実体 path を渡し、子自身に資料をReadさせ、結果はチャットで受け取ります。
