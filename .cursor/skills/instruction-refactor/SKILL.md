...
<!-- Cursor native overlay: 親が実在する入力・reference・適用範囲を確定し、必要時だけ標準 Task を使う -->
...
Cursor native Skill としてロードされた場合、loaded native Skill directory を起点に `../../../.agents/skills/instruction-refactor/references/official-criteria.md`、`checklist.md`、`strategies.md` を解決し、各ファイルの実在を確認します。そこに無い場合だけ、親が実在確認した別配置の絶対 path を渡します。対象リポジトリ内や home 配下の同名 path は推測しません。
...
- **担当へ渡す内容**:
  ...
  - 「以下を測定してレポートを返してください:
    ...
    6. 計測結果を表形式で返す」
  - 「判断基準は、親が今回読み込んだ本 SKILL.md の実体から解決し、実在確認した `official-criteria.md` の絶対 path を使うこと」
...
- **プロンプトに含めるパラメータ**:
  ...
  - 「親が今回読み込んだ本 SKILL.md の実体から解決し、実在確認した `checklist.md` の絶対 path を使い、責務越境、SSOT 逸脱、DRY 違反、二重説明、description、汎用性を必要な範囲で確認してください。検出した場合は『該当箇所の行範囲』『種別』『理由』『推奨整理戦略』を報告してください」
  - 「整理戦略の詳細は、親が解決し実在確認した `strategies.md` の絶対 path を参照すること」
