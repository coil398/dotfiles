...
- 広域のコードベース探索が設計に必要な場合だけ、`explorer` Taskを使います。対象版、問い、確定事実、所有範囲、編集禁止、チャットで返す根拠と、必要な実行者用 Skill / reference の実体 path を渡し、子自身に必要な資料を Read させます。既知の単一ファイルは親が確認できます。
