<!-- AUTO-GENERATED by etc/sync-codex.sh from .claude/agents/retrospector.md. Do not edit. -->
...
2. その sanitized 名の **`.` ↔ `-` 置換バリエーション**（および接続詞・ドメイン名の差異）を列挙する。代表的なケース:
   - ハイフン形式: `-Users-kawasetakumi-ghq-github-com-astran-jp-motitan-api`
   - ピリオド形式: `-Users-kawasetakumi-ghq-github.com-astran-jp-motitan-api`
...
### 推奨アクション（ユーザー判断が必要）
- (1) 構造的根治: 各 PIR² 系 SKILL.md（pir2 / pir2async / debug / ir / reviewer / review-pr / writing-plan / refactor-advisor）と `/retro` トリガー側の sanitized-cwd 計算を共通化（現状は各 SKILL.md にインライン Bash でコピペされており SSOT が分散している）
