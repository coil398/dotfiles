...
N5 でエージェント定義を、または N6 でスキルを Edit/Write した場合のみ実行する。「改善したつもりが悪化」を無検証でコミットするのを防ぐ軽量ゲート。retrospector はサブエージェントのため別 judge エージェントを `Agent` 起動できない。よって**実際に過去タスクを再実行するのではなく、コミット前の現行版（before）と編集後（after）を自分自身で批判的に読み比べる自己 A/B judge** を行う（opus 起動時はより厳密になる）。
