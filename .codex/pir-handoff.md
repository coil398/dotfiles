<!-- AUTO-GENERATED by etc/sync-codex.sh from .claude/pir-handoff.md. Do not edit. -->

# PIR² 引継ぎ (handoff.md) プロトコル

PIR² 系スキル（`/pir2`, `/pir2async`, `/debug`）が「複数回の実行にまたがってタスクを引き継ぐ」ための仕様。このファイルは `~/.codex/AGENTS.md` から参照される。**protocol 変更時はこのファイルを SSOT として編集する**。
...
- `PROJECT_ROOT` はスキル起動時のカレント（`$(pwd)`）。基底パスの SSOT は `~/.agents/skills/pir2/references/run-dir-base.md`
...
- スキル本体（メイン Codex）が **planner の plan.md 完成直後** に初期 handoff.md を書き出す
- 内容:
  - タスク要約（ユーザー指示から抽出）
