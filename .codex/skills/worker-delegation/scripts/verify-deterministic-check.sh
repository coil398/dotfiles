...
#   bash $PROJECT_ROOT/.codex/skills/worker-delegation/scripts/verify-deterministic-check.sh --syntax-only
#
...
REFERENCE_MD="${PROJECT_ROOT}/.codex/skills/worker-delegation/references/deterministic-completion-check.md"
...
BLOCK_RANGES_FILE="$WORK_DIR/block-ranges"
awk '
  {
    line = $0
    trimmed = line
...
' "$REFERENCE_MD" > "$BLOCK_RANGES_FILE"
BLOCK_COUNT="$(awk 'END {print NR + 0}' "$BLOCK_RANGES_FILE")"
...
echo "found ${BLOCK_COUNT} \`\`\`bash block(s) in $REFERENCE_MD"
...
echo "OK: ${BLOCK_COUNT} bash blocks syntax-valid"

if [ "$RUN_FIXTURES" -eq 0 ]; then
  exit 0
fi
...
# 結果はそのまま有効）。
if [ "$BLOCK_COUNT" -ne 3 ]; then
  echo "WARN: ${BLOCK_COUNT} bash block(s) found (fixture は 3 ブロック構成を前提としている）。フィクスチャをスキップする。verify-deterministic-check.sh の追従修正が必要。"
  exit 0
fi
