...
- 子エージェントの並列数は、アクティブ設定の `max_concurrent_threads_per_session` と実行時に利用可能な空き枠の低い方に従います。設定値を埋めることは要求せず、完了済みを空き枠と推測しません。利用可能な既存 thread は `followup_task` で再利用します。
...
3. 独立 Ti は、各 Ti に1担当・専有の所有範囲と別の `SUB_RUN_DIR` を割り当て、実効容量が許す同じ wave で並列起動します。起動前に `list_agents` で生存数と利用可能な空き枠を確認し、アクティブ設定の `max_concurrent_threads_per_session` と実行時空き枠の低い方を超えないようにします。容量が不足する場合は wave を分け、reviewer / tester も同じ規則で起動します。
