...
name: field-notes
description: >-
  実験やキャンペーンで次の判断を変える短期メモを capture / recall / triage する。方針変更やキャンペーン再開時に使い、単なる進捗・日記・過去経緯の検索には使わない。長期記憶は /ai-ltm、感想は /ai-diary。ユーザーが /field-notes と入力したら使う。
---
...
| `/ai-ltm` | 過去の経緯の検索倉庫 | 必要時検索（自動発動の条件は `/ai-ltm` の SKILL.md） |
