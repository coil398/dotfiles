...
`.agents/skills/deepplan/SKILL.md` を同一 `RUN_DIR` で実行。プロンプトに「デバッグタスクであること」「plan.md 冒頭に診断セクション（file:line 証拠必須）」を追加で渡す。EXPLORATION_NEEDED 残時の再実行も deepplan。
...
- プロンプト:
  ...
  - （`RESUME_MODE` が `new` または `resume` の場合のみ）`HANDOFF_PATH=$HANDOFF_PATH` と「実装完了した項目を handoff.md で `[x]` 化し、新規発見の TODO は追記すること。詳細: `~/.codex/pir-handoff.md`」
