...
`PLAN_MODE=deepplan` の場合は deepplan が作成した `{RUN_DIR}/plan.md` を main/primary agent が Read してから保存します。通常モードでも、追加探索後は既存planを main/primary agent が増分更新します。フォーマットは通常の PIR² と同じ（目標・実装計画・設計詳細・実装ログ）。
