...
親がスコープ、対象、担当選択、改善の実施、受入、結果の統合を保持します。委任時は、親が実在確認した `references/official-criteria.md`、`checklist.md`、`strategies.md` と必要な共有基準の物理 path、対象版、所有範囲、制約、返却形式を担当へ渡し、担当自身に必要な資料を Read させます。親が直接確認する場合は同じ資料を自分で Read します。子へこの親用の探索・判定・改善工程を渡して再起動させません。

read-only 担当は測定・構造判定と未確認範囲だけを親へ返し、対象 instruction、report、記憶を保存しません。保存や修正は親が明示した専有 path と承認範囲で行い、評価を共通結果契約へ接続する必要がある場合だけ親がその正本を使います。
...
必要な場合だけ、read-only 調査担当へ測定を委譲する。起動できない場合は親が直接測定する。委任時には親が解決・実在確認した `official-criteria.md` の物理 path を必ず渡し、担当自身に Read させる。
...
- **担当へ渡す内容**:
  ...
  - 「以下を測定してレポートを返してください:
    ...
    6. 計測結果を表形式で返す」
  - 「判断基準は、親が実在確認して渡した `references/official-criteria.md` の絶対 path を Read して使うこと」
...
- **プロンプトに含めるパラメータ**:
  ...
  - 「親が実在確認して渡した `checklist.md` の絶対 path を Read し、責務越境、SSOT 逸脱、DRY 違反、二重説明、description、汎用性を必要な範囲で確認してください。検出した場合は『該当箇所の行範囲』『種別』『理由』『推奨整理戦略』を報告してください」
  - 「親が実在確認して渡した `strategies.md` の絶対 path を Read し、整理戦略を選ぶこと」
...
`IMPLEMENT=true` の場合、今回の依頼または既存の明示承認に含まれる候補は、追加の応答待ちなしにステップ 6 へ進めます。依頼範囲外で既存承認に含まれない候補だけは、具体的な差分案・影響・受入条件を先に提示してから、以下の選択肢への応答を待ちます:
...
`all` または番号指定の場合、親が所有範囲、既存変更、受入条件を確認したうえで、直接修正するか現在のランタイムの委譲 primitive へ渡す。委任する場合も、子には対象 path と必要な専門 reference の物理 path だけを渡し、親用の再委任・統合工程を渡さない。修正結果は親が確認し、保存・report 作成・記憶追記を行う。
