...
アイデアを完全な設計へ落とし込むための対話的プロセスです。このスキル本体（= メインエージェント）がオーケストレーターとなり、必要に応じて `explorer` 等の subagent を利用中ランタイムの委譲機構で起動します。subagent 内からの再委譲可否は利用中ランタイムの仕様に従うため、起動責任はスキル本体に集約されます。
...
コードベース探索（ファイル構造の把握、既存パターンの列挙、関連コンポーネントの特定、同一レイヤーの全件列挙など）は必ず `explorer`（または利用中ランタイムの同等 read-only 探索担当）に委譲してください。メインエージェントが Glob / Grep / 複数ファイル Read / Bash の `rg`/`grep`/`find`/`fd`/`ls` による広域探索を直接呼ぶことは禁止です（詳細は利用中ランタイムのグローバルルールを参照）。利用中ランタイムにサブエージェント機構がない場合は、同じ探索単位と順序をメインエージェントが実行してください。

- **スキル本体（メインエージェント）が** 利用中ランタイムのサブエージェント機構で `explorer` を1〜3体起動し、調査対象と期待する出力を具体的に指示する。機構がない場合は、メインエージェントが同じ探索単位・順序を実行する
