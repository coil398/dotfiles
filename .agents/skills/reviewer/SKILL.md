...
明示指定があればその観点集合を使う。`--all-reviewers`なら五つの基本観点を対象にする。担当数と独立性は、依頼で明示された条件と現在のruntimeの容量から決める。未指定時は変更内容と失敗時に起きる具体的な実害から必要な観点だけを選ぶ。レビュー不要な局所変更は親のdiff照合とfocused checkで足りる場合がある。
