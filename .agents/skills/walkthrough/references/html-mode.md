...
| タイミング | 挙動 |
|---|---|
| ステップ6（冒頭ウォークスルー保存直後） | フル生成 → `Markdown ファイルリンク` → **ブラウザで開く** |
| ステップ7の詳細化ループ中 | **何もしない**。md にだけ追記する |
| ステップ7でユーザーが「HTML 見せて」「更新して」と言った | その時点の md 全体からフル再生成 → `Markdown ファイルリンク` → ブラウザで開く |
| ステップ8（終了時） | 最終版をフル再生成 → `Markdown ファイルリンク` → **開かず「タブをリロードして」と伝える**（ステップ6で開いたタブが残っているのが通常なので、タブを増やさない） |
...
テンプレートは外部リクエスト 0 なので Artifact としてそのまま公開できる。ただし 外部サービス へのアップロードは外部送信なので、**ユーザーが明示的に求めたときだけ**行い、その前に `共有用デザイン` skill を読む。既定はローカルファイルのみ。
