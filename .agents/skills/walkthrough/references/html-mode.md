...
| 情報量 | md と HTML は **同一内容**。HTML だから削る、は禁止（SKILL.md §4 で対象規模に合わせて選んだ構成をそのまま使う） |
| 保存先 | SKILL.md §5 と同じ。ユーザーまたは親が指定した保存先の配下に、md と同じ basename で置く。指定がなければ保存先を推測せず、HTML を生成しない理由を伝える |
| 生成方法 | `references/html-template.html` をコピーし、`{{...}}` と `<main>` 内だけ差し替える。CSS / JS は編集しない |
| 外部リクエスト | **0 件**。CDN・Web フォント・外部画像・`fetch` を追加しない |

外部リクエスト 0 を守る理由は、`file://` で開いてもオフラインで完全に動くこと、CSP の厳しい閲覧環境でも崩れないこと。Mermaid も外部ライブラリなので **HTML 版では使わない**（代替は §4）。
...
`<main class="wt-main">` の中身は md のセクションと 1:1 対応させる。md で省いたセクションは HTML でも省く。

| md のセクション | HTML | id |
|---|---|---|
| （タイトル・メタ行） | `<header class="wt-head">` の `{{TITLE}}` / `{{CHIPS}}` | — |
| 変更・構造の全体像 | `<section>` + `<h2>` | `overview` |
| 読む順序 | `<section>` + `.wt-rail` | `order`、各ステップ `step-1`… |
| データ・制御フロー | `<section>` + `.wt-flow` か inline SVG | `flow` |
| 影響範囲 | `<section>` + `.wt-table` | `impact` |
...
SKILL.md §5 で md を保存した直後に実行する。

1. `references/html-template.html` を Read する
2. 全文をコピーし、以下を差し替えて、md と同じ保存先の `<md と同じ basename>.html` に Write する。同名の別対象の HTML がある場合は上書きしない
...
4. 生成した HTML のパスを Markdown リンクで提示し、保存パスも文字で伝える。専用のファイル送信 API は仮定しない
5. **ブラウザで開く**（下記）

### ブラウザで開く

HTML を生成したら既定のブラウザで開く。プラットフォームごとにコマンドが違うので、次の 1 行で振り分ける（`WT_HTML` は生成した HTML の保存パス）:

```bash
WT_HTML="<保存した HTML のパス>"; if [ -n "${WSL_DISTRO_NAME:-}" ] && command -v wslview >/dev/null 2>&1; then wslview "$WT_HTML"; elif command -v open >/dev/null 2>&1; then open "$WT_HTML"; elif command -v xdg-open >/dev/null 2>&1; then xdg-open "$WT_HTML"; else echo "自動で開けませんでした。手動で開いてください: $WT_HTML"; fi
```
...
| タイミング | 挙動 |
|---|---|
| SKILL.md §5（冒頭の説明を保存した直後） | フル生成 → `Markdown ファイルリンク` → **ブラウザで開く** |
| SKILL.md §6 の詳細化中 | **何もしない**。保存する詳細化は md にだけ追記する |
| SKILL.md §6 でユーザーが「HTML 見せて」「更新して」と言った | その時点の md 全体からフル再生成 → `Markdown ファイルリンク` → ブラウザで開く |
| 終了時（詳細化ログを保存していた場合） | 最終版をフル再生成 → `Markdown ファイルリンク` → **開かず「タブをリロードして」と伝える**（冒頭で開いたタブが残っているのが通常なので、タブを増やさない） |

終了時に開かないのは、同じ `file://` を再度 `open` するとブラウザが新規タブを積む環境が多いため。ユーザーがタブを閉じていた場合は「開き直す？」と 1 行添えれば足りる。

詳細化中は HTML が md より古い状態になる。これは意図した割り切りで、**md が SSOT** なので情報は失われない。ユーザーに提示するときも「HTML は冒頭時点／最終版」と分かる形にする（フッタの生成時刻がそれを担う）。
...
SKILL.md §2 でキャッシュを再利用し、対応する HTML が存在しない場合も、md からフル生成する。

### フラグの相互作用

| 組み合わせ | 挙動 |
|---|---|
| `--html` のみ | 保存先が指定されていれば md と HTML の両方を保存する |
| `--html --no-save` | **`--no-save` を優先**する。SKILL.md のとおりチャット内だけで完結し、md も HTML も書き出さない。HTML を生成しなかったことを 1 行伝える |
| `--html --fresh` | キャッシュを使わず再調査し、md と HTML を今回未使用の path へ新規生成する |

### 外部公開

HTML は外部リクエスト 0 のローカルファイルとして扱い、外部サービスへのアップロードは行わない。
