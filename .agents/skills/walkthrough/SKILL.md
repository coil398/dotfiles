...
スキル本体（メイン Codex）がオーケストレーターとして全エージェントの起動権限を持つ。subagent同士の相互起動は行わない。
...
- gpt-5.4-mini: 広く浅い調査（構造列挙、grep 相当）。最大3体並列
- gpt-5.5: 深く読み解く調査（既存ロジックの意味理解、設計意図の把握）。最大1体
- gpt-5.5: gpt-5.5 フォールバック（間接参照追跡・メタプログラミング・複雑な状態遷移）。最大1体、他層と並列起動しない

Codex subagentの `model` パラメータで起動時に明示指定する。gpt-5.4-mini + gpt-5.5 は同ターン並列起動可。gpt-5.5 は単独起動。gpt-5.5 → gpt-5.5 のフォールバック経路は「意図を推測で補う気配がある」「間接参照が途中で切れる」「DSL / メタプログラミングで表層 grep が効かない」の兆候で発動する。

`MODE` ごとの推奨構成:

| シナリオ | 推奨構成 |
|---------|---------|
| 小規模差分（数ファイル） | gpt-5.4-mini 1体 |
| 大規模差分 / 複数レイヤー跨ぎ | gpt-5.4-mini 2〜3体（レイヤー別並列）+ 必要なら gpt-5.5 1体 |
| 単一ファイル読解 | gpt-5.4-mini 1体（呼び出し元・被呼び出し調査）+ 必要なら gpt-5.5 1体 |
| ディレクトリ読解 | gpt-5.4-mini 1〜2体（構造・外部依存）+ gpt-5.5 1体（主要ロジック） |
| 自然文領域 | gpt-5.4-mini 1体（候補ファイル洗い出し）→ gpt-5.5 1体（意味把握） |
| 間接参照 / DSL 壁 | gpt-5.5 → gpt-5.5 フォールバック |
...
昇格時はユーザーに「対象が複雑なためチームモードで調査します」と1行告知し、`Codex subagent workflows` ツールで以下の構成を起動する:

| 役割 | モデル | 責務 |
|-----|-------|------|
| 調査リード | gpt-5.5 | 全体像を把握し分岐点・経路を特定、各メンバーに調査範囲を配分 |
| 分岐調査員 × N | gpt-5.4-mini または gpt-5.5 | 割り当てられた経路を深掘り、共有コンテキストに事実を投入 |

統合役はスキル本体（メイン Codex）。調査リードは指示のみで、メンバー起動はスキル本体が行う。subagent同士の相互起動はしない。
...
- gpt-5.5 でも意図が掴めない → gpt-5.5 にフォールバック
