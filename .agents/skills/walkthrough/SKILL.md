...
親が対象、委譲、統合、受入を保持する。担当同士の相互起動は前提にしない。
...
| file | 直接読み取り（単一ファイルのピンポイント確認として許容） |
...
呼び出し元が `/pir2` の探索方針を渡している場合は、その実体から読み込んだ契約に準拠する。渡されていない場合は、現在のランタイムの委譲機構と対象のリスクに応じて選ぶ:

- 広く浅い調査: 構造列挙、参照探索、入口の確認
- 深い調査: 既存ロジックの意味理解、設計意図、間接参照の追跡
- 追加調査: 初回の事実だけで判断できない場合に、具体的な問いへ限定して行う

担当、モデル、並列数は現在のランタイムに委ねる。複数担当は所有範囲が分離している場合だけ並列化し、対象の複雑さや間接参照の不足が確認された場合にだけ追加調査を選ぶ。

`MODE` ごとの推奨構成:

| シナリオ | 推奨構成 |
|---------|---------|
| 小規模差分 | 親の直接確認、または必要な範囲の単独調査 |
| 大規模差分 / 複数レイヤー跨ぎ | 所有範囲を分けた複数調査、必要なら深い調査を追加 |
| 単一ファイル読解 | 呼び出し元・被呼び出しの確認、必要なら深い調査 |
| ディレクトリ読解 | 構造・外部依存と主要ロジックを分けて確認 |
| 自然文領域 | 候補ファイルの洗い出しから意味把握へ進む |
| 間接参照 / DSL 壁 | 具体的な未解決箇所だけ追加調査 |
...
昇格時はユーザーに対象と理由を1行告知し、現在のランタイムの委譲 primitive で、所有範囲を分けた必要な担当だけを起動する:

| 役割 | 責務 |
|-----|------|
| 調査リード | 全体像を把握し、分岐点・経路・担当範囲を整理 |
| 分岐調査員 | 割り当てられた経路を深掘り、確認済み事実を返す |

統合役は親。調査リードは割り当てられた範囲だけを扱い、担当同士の相互起動は前提にしない。
...
- 情報不足 → 具体的な問いへ限定した追加調査を行う
- 追加調査でも意図が掴めない → 不足している事実と必要なユーザー判断を報告する
...
スキル終了後もユーザーは通常チャットで親に質問を続けられる。親は最後に作成・更新したウォークスルーを参照して Q&A に応答できる。再度スキルを起動する場合はユーザーが `/walkthrough` を呼び直す。

---

## 既存スキルとの棲み分け

- `/pir2` のサマリー版ウォークスルー: 実装直後に親が必要と判断した場合だけ生成する
