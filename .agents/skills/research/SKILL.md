...
研究ワークフローを実行します。このスキル本体（= メイン Codex, Opus）がオーケストレーターとなり、explorer（調査）→ 集約（オーケストレーター自身）→ thinker（思考）→ hypothesizer（仮説）を `Agent` ツールで順に起動します。集約フェーズはサブエージェントに委譲せず、**オーケストレーター自身が探索結果を統合**します。サブエージェントも `Agent` ツールでネスト起動できますが、本ワークフローでは制御フロー（起動・ゲート管理・成果物集約・ユーザー確認）をスキル本体に集約する設計とし、サブからのネスト起動は read-only の探索（explorer）に限ります。
