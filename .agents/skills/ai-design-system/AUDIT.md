...
現状のコードベースを `IDEAL.md` のチェックリストで評価し、gapを検出するフロー。修正は `反映` モードが明示された場合だけ行う。

---

## モード境界

この資料を監査・確認・レビュー（`評価`、`audit-only`）として使う場合は読み取り専用で実行する。SSOT・コード・設定を生成・更新せず、評価結果、指摘、判定できない項目を `未確認` として返す。`Step 3` の優先度は反映候補の報告にとどめ、`Step 4` と `Step 5` は修正・更新の依頼または既存の明示承認がある `反映` モードでのみ実行する。
...
`design-system.config.*` の有無を確認し、存在する場合は読み、以下を把握する：

- 定義されているトークンの一覧
- 禁止されているアンチパターン
- コンポーネントの配置・命名規則
- **aesthetic セクション**（tone / differentiation / antiDirection）— 存在しない場合は `未確認` として記録する。`評価` モードでは `BOOTSTRAP.md` Step 1.5 を実行しない

SSOTがない場合も、コード、構成、アクセシビリティ、モーションなどSSOTを必要としない項目の評価を続ける。SSOTを前提とする項目は不在・未確認としてレポートする。
...
`評価` モードでは以下の優先度を指摘としてレポートする。修正を開始する認可にはならない。
...
## Step 4: `反映` モードで修正を実施する

修正・更新の依頼または既存の明示承認がある場合だけ実行する。`評価`（`audit-only`）ではこのStepを実行せず、Step 2 のレポートで終了する。
...
## Step 5: `反映` モードでSSOTを更新する（必要な場合）

SSOTの更新も、修正・更新の依頼または既存の明示承認がある場合だけ実行する。`評価`（`audit-only`）では更新案を指摘として返し、ファイルは変更しない。
...
新規コンポーネントを実装するとき、または既存コンポーネントを拡張するときに必ず確認するパターン。design-review の観点6（コンポーネント単位のピクセル整合性）と対になる、**実装側の予防チェック**。
